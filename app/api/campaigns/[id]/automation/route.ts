...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  if (!patchParsed.success) {
    return NextResponse.json(
      { error: 'Configuração inválida', details: patchParsed.error.issues },
      { status: 400 },
    )
  }
  ...
  try {
    ...
    try {
      ...
      if (!UPDATABLE_STATUSES.has(campaign.status)) {
        return NextResponse.json(
          { error: `Configuração so pode ser alterada enquanto a campanha esta em execucao (status: ${campaign.status})` },
          { status: 409 },
        )
      }
      ...
      log.info({ campaignId, patch }, 'Configuração de automacao atualizada')
    ...
    if (res) return res
    log.error({ err }, 'Erro ao atualizar configuração de automacao')
