...
//
// Allowed from: draft | awaiting_confirmation | awaiting_channel | awaiting_message | ready_to_send
// Not allowed when: sending | completed | completed_with_errors | cancelled
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    ...
    try {
      ...
      if (!CANCELLABLE_STATUSES.has(campaign.status)) {
        return NextResponse.json(
      ...
      const updated = await updateCampaignStatus(client, campaignId, 'cancelled')
